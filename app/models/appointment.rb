class Appointment < ApplicationRecord
  ...
  scope :future_vet_apts, -> (v) { self.future_date_apts.where('appointments.vet_id = ?', v.id) + self.future_time_apts.where('appointments.vet_id = ?', v.id) }
  scope :prev_vet_apts, -> (v) { self.prev_date_apts.where('appointments.vet_id = ?', v.id) + self.prev_time_apts.where('appointments.vet_id = ?', v.id) }
  scope :future_pet_apts, -> (p) { self.future_date_apts.where('appointments.pet_id = ?', p.id) + self.future_time_apts.where('appointments.pet_id = ?', p.id) }
  scope :prev_pet_apts, -> (p) { self.prev_date_apts.where('appointments.pet_id = ?', p.id) + self.prev_time_apts.where('appointments.pet_id = ?', p.id) }
